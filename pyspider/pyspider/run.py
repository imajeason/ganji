...
@click.option('--beanstalk', envvar='BEANSTALK_HOST',
              ...
              'please use --message-queue instead.')
@click.option('--firefox-proxy', envvar='firefox_PROXY', help="firefox proxy ip:port")
...
def cli(ctx, **kwargs):
    ...
    # firefox-proxy
    if kwargs.get('firefox_proxy'):
        pass
    elif os.environ.get('FIREFOX_NAME'):
        kwargs['firefox_proxy'] = os.environ['FIREFOX_PORT_25555_TCP'][len('tcp://'):]
...
@click.option('--firefox-endpoint', help="endpoint of FIREFOX, start via pyspider FIREFOX")
...
def fetcher(ctx, xmlrpc, xmlrpc_host, xmlrpc_port, poolsize, proxy, user_agent,
            ...
            async=False, get_object=False, no_input=False):
    """
    Run Fetcher.
    """
    g = ctx.obj
...
@cli.command()
@click.option('--firefox-path', default='firefox', help='firefox path')
@click.option('--port', default=25555, help='firefox port')
@click.option('--auto-restart', default=False, help='auto restart firefox if crashed')
@click.argument('args', nargs=-1)
@click.pass_context
def firefox(ctx, firefox_path, port, auto_restart, args):
    """
    Run firefox fetcher if firefox is installed.
    """
    ...
    g = ctx.obj
    _quit = []
    firefox_fetcher = os.path.join(
        os.path.dirname(pyspider.__file__), 'fetcher/firefox_fetcher.js')
    cmd = [phantomjs_path,
...
def all(ctx, fetcher_num, processor_num, result_worker_num, run_in):
    ...
    try:
        # firefox
        if not g.get('firefox_proxy'):
            firefoxjs_config = g.config.get('firefoxjs', {})
            firefoxjs_config.setdefault('auto_restart', True)
            threads.append(run_in(ctx.invoke, firefox, **firefoxjs_config))
            time.sleep(2)
            if threads[-1].is_alive() and not g.get('firefox_proxy'):
                g['firefox_proxy'] = '127.0.0.1:%s' % firefoxjs_config.get('port', 45501)
...
@click.option('--firefox', 'enable_firefox', default=False, is_flag=True,
              help='enable firefox, will spawn a subprocess for firefox')
@click.argument('scripts', nargs=-1)
@click.pass_context
def one(ctx, interactive, enable_firefox, scripts):
    """
    ...
    if enable_firefox:
        firefox_config = g.config.get('firefox', {})
        firefox_obj = ctx.invoke(firefox, **firefox_config)
        if firefox_obj:
            g.setdefault('_proxy', '127.0.0.1:%s' % firefox_obj.port)
    else:
        firefox_obj = None
